...
function hodUsername($depart)
{
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "SELECT * FROM audcomment WHERE courseName='$courseName'  AND docType='$docType' AND audUsername='$audUsername' ";
        $result = $conn->query($sql);
        $data = mysqli_query($conn, $sql);
        $total = mysqli_num_rows($data);
        if ($total != 0) {
            ...
            $insert= "INSERT INTO `audcomment`( `courseID`, `academicYear`, `dept`, `year`, `sem`, `courseName`, `subjectCode`, `teaUsername`, `hodUsername`, `role`, `audUsername`, `docType`, `comment`, `date&time`) VALUES ('$courseID','$academicYear','$dept','$year','$sem','$courseName','$subjectCode','$teaUsername','$hodUsername','$role','$audUsername','$docType','$comment','$date')";
...
<div class="bs-example">
    ...
    <div id="navbarCollapse" class="collapse navbar-collapse">
        ...
        <a href="http://localhost/FAS/teacherhomelab.php?username=<?php echo $audUsername; ?>"><img src="../../css/images/dylogo.png" style="width:10rem; height:4rem" ></a>
        </div>

        <div class="navbarCenter">
            <h4 style="font-weight:1000">Inter-Departmental Auditor Response (Lab)</h4>
        </div>
    ...
    <form action="lab.php?username=<?php echo $audUsername; ?>&teaUsername=<?php echo $teaUsername;?>&courseID=<?php echo $courseID; ?>&subjectCode=<?php echo $subjectCode; ?>" method="post">
        <main>
